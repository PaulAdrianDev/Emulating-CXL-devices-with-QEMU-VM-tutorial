...
// CUSTOM CODE
static void log_folio_user_vaddr(struct folio *folio, unsigned long old_phys, int reason)
{
    unsigned long phys = folio_pfn(folio) << PAGE_SHIFT;
    pr_info("MIGRATION:0x%lx,0x%lx,%d\n", old_phys, phys, reason);
}
			      ...
			      struct list_head *ret)
{
	...
	if (reason == MR_DEMOTION || reason == MR_NUMA_MISPLACED)
        	log_folio_user_vaddr(dst, folio_pfn(src) << PAGE_SHIFT, reason);
	//
